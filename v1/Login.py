...
def get_db_connection():
    try:
        connection = mysql.connector.connect(
            host="localhost",
            user="root",
            password="Letuan191003+",  # Consider using environment variables or external configurations
            database="loginapp"
        )
        return connection
...
def user():
    ...
    if request.method == 'POST' and 'photo' in request.files:
        ...
        uploaded_image_path = url_for('static', filename='uploads/' + file.filename)
    ...
    if request.method == 'POST':
        ...
        if connection:
            try:
                ...
                cursor.execute("INSERT INTO list_user VALUE (%s, %s)", (username, password))
                connection.commit()
